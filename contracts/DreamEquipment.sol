...
contract DreamEquipment is ERC721, Operator, IEquipment {
    ...
    function setEquipped(uint256 id, bool isEquipped) public {
        Equipment storage equip = equipment[id];
    ...
    function ownerOf(uint256 id) public view override(ERC721, IEquipment) returns (address) {
     ...
     *
     */
    function getEquipment(uint256 id) public view returns (Equipment memory) {
        return equipment[id];
    }
    ...
    function createEquipment(address to, string memory name, uint32[] memory values) public onlyOperator {
        ...
        equipment[index] = Equipment(name, index, values[0], values[1], values[2], values[3], values[4], values[5], false);

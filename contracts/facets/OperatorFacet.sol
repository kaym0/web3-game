...
contract OperatorFacet  {
    ...
    modifier onlyMasterOperator {
        if (msg.sender != LibDiamond.diamondStorage().masterOperator) revert NotMasterOperator();
        _;
    }
    ...
    function transferMasterOperator(address operator) public onlyMasterOperator {
        LibDiamond.diamondStorage().masterOperator = operator;
    ...
    function relinquishOwnership() public onlyMasterOperator {
        LibDiamond.diamondStorage().masterOperator = address(0);
    ...
    function removeOperator(address operator) public onlyOperator {
        if ( LibDiamond.diamondStorage().masterOperator == operator) revert CannotRemoveMasterOperator();
        if ( LibDiamond.diamondStorage().operatorsCanWrite == false &&  LibDiamond.diamondStorage().masterOperator != msg.sender) revert InsufficientPrivileges();
         LibDiamond.diamondStorage().operators[operator] = false;
    ...
    function addOperator(address operator) public onlyOperator {
        if ( LibDiamond.diamondStorage().operatorsCanWrite == false &&  LibDiamond.diamondStorage().masterOperator != msg.sender) revert InsufficientPrivileges();
         ...
         LibDiamond.diamondStorage().operators[operator] = true;
